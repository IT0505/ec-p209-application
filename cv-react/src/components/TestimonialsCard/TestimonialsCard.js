...
function TestimonialsCard() {
                ...
                <Swiper pagination={{"clickable": true}} className="mySwiper">
                    {clientData.map((data, index) =>         
                        <SwiperSlide className="client-content" key={index}>
                            ...
                            <blockquote className="client-description">{data.description}</blockquote>
    ...
    const clientLogosView = () => {
        return (
            <div className="autoSlider-wrap" style={{'--loadSlider': `${loadSlider}px`}}>
    ...
    useEffect(() => {
        if($(".client-list") && $(".autoSlider-wrap")) {
            setLoadSlider($(".client-list").width()-$(".autoSlider-wrap").width())
        }
    }, [])
